...
def format_train_checkins(file_train, u_dict, p_dict, user_visit_dict, train_poilist, file_out, emb_dim):
    # 开始格式化签到记录
    j = 0
    # 304047*2
    pos_emb_matrix = np.zeros((emb_dim,304047))
    neg_emb_matrix = np.zeros((emb_dim,304047))
    ...
    for line in file_train.readlines():
        ...
        pid = int(info[5])
        if j < 304047:
            pos_emb_matrix[0:,j] = np.concatenate((u_dict[uid],p_dict[pid]),axis=0)
            if j == 0 or j == 304046:
            ...
            neg_emb_matrix[0:,j] = np.concatenate((u_dict[uid],p_dict[sample_poi]),axis=0)
            if j == 0 or j == 304046:
...
def format_tune_checkins(file_tune, u_dict, p_dict, file_out, emb_dim):
    # 开始格式化签到记录
    j = 0
    # 33359
    emb_matrix = np.zeros((emb_dim, 33359))
    for line in file_tune.readlines():
        info = line.split("	")
        uid = int(info[0])
        # time = int(info[4][11:13])
        pid = int(info[5])
        emb_matrix[0:,j] = np.concatenate((u_dict[uid],p_dict[pid]),axis=0)
        if j == 0 or j == 33358:
...
if __name__ == '__main__':
    ...
    # 写代码真的还有很长一段路要走啊
    # file_time = open("../t_data/matrix/t_smooth_train_time_sim_matrix.csv","r",encoding="utf-8")
    # file_user_visit = open("../t_data/train/t_train_user_visit.txt","r",encoding="utf-8")
    file_user_visit = open("../t_data/valid_total/t_valid_total_user_visit.txt","r",encoding="utf-8")
    ...
    for i in dim_list:
        ...
        file_train = open("../t_data/t_train_set.txt","r",encoding="utf-8")
        # file_tune = open("../t_data/t_tune_set.txt","r",encoding="utf-8")
        file_user = open("../t_data/embeddings/user/{0}user_embedding.csv".format(i),"r",encoding="utf-8")
        file_poi = open("../t_data/embeddings/poi/{0}poi_embedding.csv".format(i),"r",encoding="utf-8")
        file_out1 = open("../t_data/train_format_input_notime/t_train_format_{0}embedding_lapless.csv".format(i),"w",encoding="utf-8")
        # file_out2 = open("../t_data/tune_format_input_notime/t_tune_format_{0}embedding.csv".format(i),"w",encoding="utf-8")
        # user和poi的embedding字典【是随着维度不同而变化的】
        u_dict = create_emb_dict(file_user)
        p_dict = create_emb_dict(file_poi)
        emb_dim = 48+(4*i)
        # format_tune_checkins(file_tune, u_dict, p_dict, file_out2, emb_dim)
        format_train_checkins(file_train, u_dict, p_dict, user_visit_dict, train_poilist, file_out1, emb_dim)
        # file_out2.close()
        file_out1.close()
        file_poi.close()
        file_user.close()
