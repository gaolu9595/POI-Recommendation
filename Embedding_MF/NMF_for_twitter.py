# 对Twitter数据集构造的poi的共现矩阵、地理位置矩阵进行NMF
# 相比较于gowalla_5months数据集而言，这里是没有social矩阵的
...
def doNMF(matrix,type,dim):
    ...
    if type == 0:
        # 在Twitter数据集中没有social矩阵
        np.savetxt("../t_data/nmf_result/social/{0}user_social_feature.csv".format(dim),featureVectors,delimiter=",",fmt="%f")
    elif type == 1:
        np.savetxt("../t_data/nmf_result/geo/{0}poi_geo_feature.csv".format(dim),featureVectors,delimiter=",",fmt="%f")
    else:
        np.savetxt("../t_data/nmf_result/covisit/{0}poi_covisit_feature.csv".format(dim),featureVectors,delimiter=",",fmt="%f")
...
if __name__ == '__main__':
    ...
    # file_social_matrix = "../t_data/matrix/t_total_user_social_matrix.txt"
    ...
    # 矩阵文件,读取矩阵信息,做非负矩阵分解
    file_poi_geo_matrix = "../t_data/matrix/t_valid_total_poi_sim_matrix.txt"
    ...
    # file_poi_covisit_matrix = "../t_data/matrix/t_train_co_visit_matrix.txt"
