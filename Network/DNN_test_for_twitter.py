...
def result_for_topk(timelist,time_dict,time_user_dict,user_visit_already,u_dict,p_dict,userlist,poilist):
    my_dnn_model = load_model("./dnn_version/twitter_dnn_trained_300dim_ver2.h5")
    k = 50      # Top_K推荐
    for i in timelist:
        ...
        for j in range(size):
            ...
            # 当generator是Sequence的一个实例时，steps可以为none        #10768
            ...
            topk_pois = []
        file_out = open("../t_data/test/recommendation/ver1/300result_{0}time.txt".format(i),"w",encoding="utf-8")
