...
if __name__ == '__main__':
    print("=============================BuildModel===============================")
    epochs = 200     # 迭代次数【感觉迭代次数尽可能多点吧，但过多可能就过拟合了？】
    ...
    model = Sequential()
    # model.add(BatchNormalization(input_shape=(472,)))
    ...
    log_path = "./logs/log_%s"%(time)

    # after normalization
    print("=============================StartReadingData===============================")
    file_train = "../f_data/train_format_input/threshold/f_train_format_300embedding.csv"
    file_tune = "../f_data/tune_format_input/threshold/f_tune_format_300embedding.csv"
    x_train, y_train = loadTrainData(file_train)
    x_tune, y_tune = loadTuneData(file_tune)
    print("=============================StartNormalization=====================================")
    ...
    print("=============================StartTraining=====================================")
    tensorboard = callbacks.TensorBoard(log_dir=log_path, histogram_freq=1, write_graph=True)
    early_stop = callbacks.EarlyStopping(monitor="loss", min_delta=0.001, patience=10, mode="auto", verbose=1)
    checkpoint = callbacks.ModelCheckpoint("./dnn_version/threshold_256_nobn/weights.{epoch:02d}-{val_loss:.2f}.hdf5", monitor="val_loss", verbose=1, save_best_only=True, save_weights_only=False, mode="auto", period=1)
    model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, validation_data=(x_tune, y_tune), shuffle=True, verbose=1, callbacks=[tensorboard, early_stop, checkpoint])
    print("=============================StartTuning=====================================")
