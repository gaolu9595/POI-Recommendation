...
def result_for_topk(timelist,time_dict,time_user_dict,user_visit_already,u_dict,p_dict,userlist,poilist):
    my_dnn_model = load_model("./dnn_version/f_neighbor/weights.09-0.46.hdf5")
    k = 20      # Top_K推荐
    for i in timelist:
        ...
        for j in range(size):
            ...
            # 当generator是Sequence的一个实例时，steps可以为none        # 16542
            # batch_testdata_generator = My_class.DataGenerator(user_emb, p_dict, time_emb, poilist)
            print("Start Generating data……")
            rawresult = my_dnn_model.predict_generator(__data_generation(user_emb, p_dict, time_emb, poilist), steps=int(len(poilist)/100)+1, verbose=1)
            # result是一个打分的numpy数组
            ...
            topk_pois = []
        file_out = open("../f_data/test/recommendation/f_neighbor/300result_{0}time.txt".format(i),"w",encoding="utf-8")
...
if __name__ == '__main__':
    ...
    file_train_user_visit = open("../f_data/train/f_train_user_visit.txt","r",encoding="utf-8")
    ...
    file_user = open("../f_data/embeddings/user/neighbor/300user_embedding.csv","r",encoding="utf-8")
    file_poi = open("../f_data/embeddings/poi/neighbor/300poi_embedding.csv","r",encoding="utf-8")
